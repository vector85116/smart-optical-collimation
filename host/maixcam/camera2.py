...
# 第二个摄像头版本：只控制水平移动，只输出 left only / right only / stop
DIRECTION_MAP = {
    (-1, 0): "right only",
    (1, 0): "left only",
    (0, 0): "stop",
...
# UART 只允许发送这三种指令，missing / up / down / 斜向都不会发给下位机
UART_ALLOWED_DIR_CODES = ("left only", "right only", "stop")
...
# False：串口只发 dir_code，例如 left only / right only / stop
# True：串口发完整调试文本
# 注意：本版本建议保持 False，确保下位机只收到三种水平控制指令
...
def make_control_result(ring, spot):
    ...
    # 第二个摄像头版本：只计算水平偏差 dx，不再用 dy 控制上下方向
    dx = int(result["spot_x"] - result["ring_x"])
    sx = sign_with_deadzone(dx, DIRECTION_DEADZONE_PX)

    # sy 固定为 0，因此 dir_code 只可能是 right only / left only / stop
    sy = 0
...
def main():
    ...
    print("horizontal only: left only | right only | stop")
    print("UART allowed dir_code: {}".format(UART_ALLOWED_DIR_CODES))
    ...
    while True:
        ...
        if serial is not None and (not uart_stopped) and now_ms - last_uart_ms >= UART_SEND_PERIOD_MS:
            ...
            if dir_code in UART_ALLOWED_DIR_CODES:
                ...
                # 没识别到 ring / laser 时是 missing，不发送给下位机，并清空 stop 连续计数
                stop_send_count = 0
